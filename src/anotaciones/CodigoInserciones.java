    ...
    public static void Init(Object instance, Class claseRepresentada, Annotation annotacion, Method method) throws Exception {
        ...
        for (Field f : claseRepresentada.getDeclaredFields()) {
            try {
                ...
                if (!f.isAnnotationPresent(NoInit.class) && f.getType().equals(Integer.TYPE))
                {
                    f.set(instance, c.Integer());
                } else if (!f.isAnnotationPresent(NoInit.class) && f.getType().equals(Double.TYPE))
                {
                    f.set(instance, c.Double());
                } else if (!f.isAnnotationPresent(NoInit.class) && f.getType().equals(Character.TYPE))
                {
                    f.set(instance, c.Char());
                } else if (!f.isAnnotationPresent(NoInit.class) && f.getType().equals(String.class))
                {
                    f.set(instance, c.String());
                } else if (f.getType().equals(Boolean.TYPE))
                {
                    f.set(instance, c.Boolean());
                } else if (!f.isAnnotationPresent(NoInit.class) && f.getType().equals(Float.TYPE))
                {
                    f.set(instance, c.Float());
                } else if (!f.isAnnotationPresent(NoInit.class) && f.getType().equals(Long.TYPE))
                {
